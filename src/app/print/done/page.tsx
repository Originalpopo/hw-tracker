...
export default function PrintDonePage() {
  ...
  const loadData = async (name: string) => {
    try {
      ...
      // Filter only tasks that are 'Done' (ทำเสร็จแล้ว - รอส่ง / รอตรวจ)
  ...
  const uniqueSubjects = useMemo(() => {
    ...
    doneTasks.forEach(t => { if (t.subject?.trim()) subs.add(t.subject.trim()); });
  ...
  }, [doneTasks, filterType, filterSubject]);
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-[#e2e8f0]">
          ...
          <div className="flex items-center bg-[#eff2f7] p-1 rounded-xl border border-[#cbd3e0]">
            ...
            >
              ...
              <span>ทั้งหมด ({doneTasks.length})</span>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <span>นำไปส่งครู ({doneTasks.filter(t => !!t.teacher_column_id).length})</span>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <span>งานส่วนตัว ({doneTasks.filter(t => !t.teacher_column_id).length})</span>
            </button>
          </div>
                ...
                <tbody>
                  {subjectTasks.map((task) => (
                    <tr key={task.id} className="border-b border-black">
                      {/* Checkbox for kid to tick when handing in to teacher */}
